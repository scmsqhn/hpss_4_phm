...
class MelFilterbank(Filterbank):
    ...
    def __init__(
        ...
        if sample_rate is None and max_freq is None:
            raise ValueError(
                "Either max_freq or sample_rate should be specified."
                ", but both are None."
            )
...
def Melspectrogram(
    ...
    **kwargs
    ...
    # keunwoo: Why is num_freqs specified like this and not by the passed
    # argument?
    ...
    return nn.Sequential(
        *Spectrogram(power=2.0, **kwargs), ApplyFilterbank(mel_fb_matrix)
    )
...
class AmplitudeToDb(_ModuleNoStateBuffers):
    ...
    def __init__(self, ref=1.0, amin=1e-7):
        ...
        self.amin = amin
        assert ref > amin, (
            "Reference value is expected to be bigger than amin, but I have"
