...
def _mel_to_hertz(mel, htk):
    ...
    return torch.where(
        mel >= min_log_mel, min_log_hz * torch.exp(logstep * (mel - min_log_mel)), hz
    )
...
def _hertz_to_mel(hz, htk):
    ...
    if htk:
        return 2595.0 * torch.log10(
            torch.tensor(1.0, dtype=torch.get_default_dtype()) + (hz / 700.0)
        )
    ...
    return torch.where(
        hz >= min_log_hz, min_log_mel + torch.log(hz / min_log_hz) / logstep, mel
    )
...
def stft(
    ...
    complex_specgrams = complex_specgrams.reshape(
        leading_dims + complex_specgrams.shape[1:]
    )
...
def create_mel_filter(num_freqs, num_mels, min_freq, max_freq, htk):
    ...
    down_slopes = (-1.0 * slopes[:, :-2]) / f_diff[:-1]  # (num_freqs, num_mels)
    up_slopes = slopes[:, 2:] / f_diff[1:]  # (num_freqs, num_mels)
    mel_filterbank = torch.clamp(torch.min(down_slopes, up_slopes), min=0.0)
...
def phase_vocoder(complex_specgrams, rate, phase_advance):
    ...
    time_steps = torch.arange(
        0, complex_specgrams.size(-2), rate, device=complex_specgrams.device
    )

    alphas = torch.remainder(
        time_steps, torch.tensor(1.0, device=complex_specgrams.device)
    )
...
def amplitude_to_db(x, ref=1.0, amin=1e-7):
    ...
    return 10.0 * (
        torch.log10(x)
        - torch.log10(
            torch.tensor(ref, device=x.device, requires_grad=False, dtype=x.dtype)
        )
    )
...
def mu_law_encoding(x, n_quantize=256):
    ...
    if not x.dtype.is_floating_point:
        x = x.to(torch.float)
    mu = torch.tensor(
        n_quantize - 1, dtype=x.dtype, requires_grad=False
    )  # confused about dtype here..
...
def mu_law_decoding(x_mu, n_quantize=256, dtype=torch.get_default_dtype()):
    ...
    if not x_mu.dtype.is_floating_point:
        x_mu = x_mu.to(dtype)
    mu = torch.tensor(
        n_quantize - 1, dtype=x_mu.dtype, requires_grad=False
    )  # confused about dtype here..
    x = (x_mu / mu) * 2 - 1.0
