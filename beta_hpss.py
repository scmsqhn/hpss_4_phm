...
class HPSS(nn.Module):
    ...
    def forward(self, mag_specgrams):
        return hpss(
            mag_specgrams, self.kernel_size, self.power, self.hard, self.mask_only
        )
...
def hpss(mag_specgrams, kernel_size=31, power=2.0, hard=False, mask_only=False):
    ...
    (https://www.researchgate.net/\
        publication/254583990_HarmonicPercussive_Separation_using_Median_Filtering).
    ...
    Returns:
        ret (Tuple): A tuple of four
            ...
            ret[0]: magnitude spectrograms \
                    - harmonic parts (Tensor, in same size with `mag_specgrams`)
            ret[1]: magnitude spectrograms \
                    - percussive parts (Tensor, in same size with `mag_specgrams`)
            ret[2]: harmonic mask (Tensor, in same size with `mag_specgrams`)
            ret[3]: percussive mask (Tensor, in same size with `mag_specgrams`)
    """

    def _enhance_either_hpss(
        mag_specgrams_padded, out, kernel_size, power, which, offset
    ):
        """
        ...
        if which == "harm":
            for t in range(out.shape[3]):
                out[:, :, :, t] = torch.median(
                    mag_specgrams_padded[:, :, offset:-offset, t : t + kernel_size],
                    dim=3,
                )[0]
            ...
            for f in range(out.shape[2]):
                out[:, :, f, :] = torch.median(
                    mag_specgrams_padded[:, :, f : f + kernel_size, offset:-offset],
                    dim=2,
                )[0]
        else:
    ...
    harm, perc, ret = (
        torch.empty_like(mag_specgrams),
        torch.empty_like(mag_specgrams),
        torch.empty_like(mag_specgrams),
    )
    mag_specgrams_padded = F.pad(mag_specgrams, pad=pad, mode="reflect")

    _enhance_either_hpss(
        mag_specgrams_padded,
        out=perc,
